...
# Fit the tree_model using train_data OR PERFORM CROSS-VALIDATION
tree_model <- randomForest(target~., train, ntree=25, importance=TRUE)

# Check how the model is doing using test_data
test$pred <- predict(tree_model,test,type = "class")

# Save/Load model
FILE <- 'rf_25.rda'
save(tree_model, file = FILE)
load(FILE)
